...
const PRERENDER_DIR = path.join(__dirname, 'prerender');
...
// Функция для получения пути к предварительно отрендеренному файлу
const getPrerenderedPath = (url) => {
    // Убираем trailing slash (кроме корневого пути)
    let cleanUrl = url === '/' ? url : url.replace(/\/$/, '');

    // Маппинг URL -> путь к файлу в папке prerender
    const urlToFileMap = {
        '/': 'main.html',
        '/cases': 'cases.html',
    ...
    if (relativeFilePath) {
        return path.join(PRERENDER_DIR, relativeFilePath);
    }

    return null;
};

// Функция для проверки, является ли запрос HTML страницей
...
const server = http.createServer((req, res) => {
    ...
    // Сначала проверяем, есть ли предварительно отрендеренная версия для этого URL
    const prerenderedPath = getPrerenderedPath(req.url);

    if (prerenderedPath && isHtmlRequest(req)) {
        console.log(`🔍 Checking for prerendered file: ${prerenderedPath}`);

        // Проверяем существование предварительно отрендеренного файла
        if (fs.existsSync(prerenderedPath)) {
            filePath = prerenderedPath;
            isPrerendered = true;
            console.log(`✨ Found prerendered file: ${filePath}`);
        } else {
            console.log(`❌ Prerendered file not found: ${prerenderedPath}`);
        }
    }

    // Если предварительно отрендеренный файл не найден, используем старую логику
    if (!isPrerendered) {
    ...
    fs.access(filePath, fs.constants.F_OK, (err) => {
        if (err) {
            ...
            // Если предварительно отрендеренный файл не найден и это HTML запрос,
            // пытаемся отдать index.html (для SPA)
            if (!isPrerendered && (extname === '' || extname === '.html')) {
                const fallbackPath = path.join(DIST_DIR, 'index.html');
                console.log(`🔄 Trying fallback to index.html: ${fallbackPath}`);
                filePath = fallbackPath;
            } else {
                // Возвращаем 404 для статических файлов
                console.log(`🚫 Returning 404 for file: ${req.url}`);
                const requestTime = Date.now() - requestStartTime;
                logRequest(req, 404, filePath, err);
        ...
        fs.readFile(filePath, (error, content) => {
            ...
            if (error) {
                ...
                if (error.code === 'ENOENT') {
                    logRequest(req, 404, filePath, error);
                    console.log(`⏱️  Request completed in ${requestTime}ms`);
                    res.writeHead(404, {'Content-Type': 'text/plain'});
                    res.end('404 - Файл не найден');
                } else {
                    logRequest(req, 500, filePath, error);
                    console.log(`⏱️  Request completed in ${requestTime}ms`);
                    res.writeHead(500, {'Content-Type': 'text/plain'});
                ...
                logRequest(req, 200, filePath, null, content.length);
                console.log(`⏱️  Request completed in ${requestTime}ms`);

                res.writeHead(200, headers);
                res.end(content, 'utf-8');

                console.log(`✅ Response sent successfully${isPrerendered ? ' (prerendered)' : ''}`);
            }
        });
    });
});
...
server.listen(PORT, () => {
    ...
    console.log(`   📂 prerender: ${PRERENDER_DIR}`);
    console.log(`📹 Видео файлы: поместите в папку public/`);
    console.log(`✨ Предварительно отрендеренные страницы:`);
    console.log(`   🇷🇺 /, /cases, /manufacturing, /catalog, /catalog/*`);
    console.log(`   🇬🇧 /en, /en/cases, /en/manufacturing, /en/catalog, /en/catalog/*`);
